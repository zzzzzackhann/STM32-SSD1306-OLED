/* Turns display fully on for 5 seconds, then turns it off. */
...
int main(void) {
    ...
    /* Note that here you're putting over 1KB into RAM.
    STM32 has 128KB so it's fine,
    but for smaller microcontrollers it could be a lot. */

    uint8_t fill_screen_buf[1025];
    fill_screen_buf[0] = 0x40; // data stream, not a command
    for(uint16_t i = 1; i < 1025; i++){
        fill_screen_buf[i] = 0xFF; // filling every pixel with full value
    }
     
    i2c_write(0x3C, fill_screen_buf, 1025);
